...
const { getUpstreamRegistry } = require('./upstream')
const app = express().disable('x-powered-by').set("json spaces", 4)
const server = http.createServer(app)
const { "registries-server": config={}, description, version } = require('../package.json')

const PORT = Number(process.env.PORT || 80)
const REGISTRY_REPOSITORY = process.env.REGISTRY_REPOSITORY || config["registry-repository"]
      || die("Provide a valid registry-repository in package.json")
